...
# ╔═╡ b54f9dec-96dd-11eb-3584-ab3ea3259136
begin
	using XLSX: XLSX
	using ExcelFiles: load
end

# ╔═╡ d29e6704-7ac1-4b1e-a847-1122c39318e5
using Statistics: mean

# ╔═╡ ab3ad436-ab98-4627-b15c-5c96a2b5ee25
using CSV: CSV

# ╔═╡ c8b26838-7f68-4d42-9a46-a94d6e6e5e3f
using CategoricalArrays: CategoricalArrays, categorical

# ╔═╡ ac835b58-06d1-449b-b676-3c7e6227a5fd
using DataFrameMacros, DataFrames

# ╔═╡ 42c95312-62c5-4d9e-9853-8a5f54767a48
using Chain: @chain

# ╔═╡ 03898d45-0ad4-46c3-9914-1d233e8393c9
# ╠═╡ skip_as_script = true
#=╠═╡
using PlutoUI: TableOfContents
  ╠═╡ =#

# ╔═╡ b9a7b808-3177-4cb1-b09a-7937c8d9e7f2
# ╠═╡ skip_as_script = true
#=╠═╡
using AlgebraOfGraphics: data, visual, mapping, draw
  ╠═╡ =#

# ╔═╡ de2e4697-fde2-41d5-8eb2-76c00cb0eb42
# ╠═╡ skip_as_script = true
#=╠═╡
using CairoMakie: Lines
  ╠═╡ =#

# ╔═╡ 36b4f844-5ceb-4f26-ab44-7bfdb080c24e
begin
	using DataDeps: DataDep, @datadep_str, register, unpack
	...
	register(DataDep(
   		...
   		"",
 			...
 			["04316134950246ff42158bedcb36ac773b27c7d6a864684c1489ccfb85bcf5d7"]
	))
end

# ╔═╡ f5a3e7df-db26-43a3-b9e4-bc7c2dcdebb1
md"""
# House Prices and Interest Rates (from Shiller)
"""

# ╔═╡ 400dadcf-08d0-4eaf-9e48-5e4604b3f4dd
#@chain shiller_annual() begin
#	@subset(:year >= 1960)
#	CSV.write(joinpath(datadep"shiller", "shiller-annual.csv"), _)
#end

# ╔═╡ 168a8708-6624-43cf-832e-915168b73aac
md"""
## Download data
"""

# ╔═╡ f7d91eab-068f-4fb7-9cc8-09fe6f41dd64
shiller_houses = joinpath(datadep"shiller", "Fig3-1.xls")

# ╔═╡ 9e81a812-043c-4709-9dd2-310dd01a2a97
shiller_interest = joinpath(datadep"shiller", "chapt26.xlsx")

# ╔═╡ 52801bfc-7f38-4ecc-ab55-097d75f778c6
md"""
## House prices
"""

# ╔═╡ df196fcf-5a51-49e3-bebc-e2f63bafe143
begin
	hpi_df = DataFrame(load(shiller_houses, "Data!H7:I863", header=true))
	rename!(hpi_df, :Date => :date, Symbol("From fig2.1Revised2011.xls") => :hpi)
	
	rhpi_df = DataFrame(load(shiller_houses, "Data!A7:B863", header=true))
	rename!(rhpi_df, :Date => :date, :Index => :rhpi)
	
	hpi_df = outerjoin(hpi_df, rhpi_df, on = :date)
	
	disallowmissing!(hpi_df)
end

# ╔═╡ 8ca7feba-f1fb-450f-898e-4a97bc5065b8
annual_hpi_df0 = @chain begin
	load(shiller_houses, "Data!C7:F137")
	DataFrame
	@transform!(:Date = Int(:Date))
	rename!(
		:Date => :year, 
		:Index => :rbuildingcosts, 
		:Millions => :population, 
		Symbol("Long Rate") => :r
	)
end

# ╔═╡ bc6cea69-a21f-4979-a9a1-9522743f66c9
annual_hpi_df = @chain hpi_df begin 
	@transform(:year = floor(Int, :date))
	groupby(:year)
	combine([:hpi, :rhpi] .=> mean, renamecols = false)
	outerjoin(_, annual_hpi_df0, on = :year)
end

# ╔═╡ 27658979-b1fa-4729-a754-f9b63f517919
md"""
## Stock market and interest rates
"""

# ╔═╡ 3d60c3a8-231a-4303-ab3e-e661c374628c
realrate_df0 = @chain begin
	XLSX.readdata(shiller_interest, "Data", "A3:H152")
	DataFrame(:auto)
end

# ╔═╡ e488dc16-b05e-4662-81fa-e166d59bde97
begin
	na = first(select(realrate_df0, Not(1)))
	n = last.(pairs(na))
	
	realrate_labels = @chain realrate_df0[2:6, :] begin
		rename(:x1 => "year", pairs(na)...)
		transform(n .=> ByRow(x -> coalesce(x, "")), renamecols = false)
		combine(n .=> x -> string.(strip(join(x, " "))), renamecols = false)
		only
		pairs
	end
end

# ╔═╡ 4dcac828-9e18-466c-9eb2-b38c6832e286
realrate_df = @chain realrate_df0 begin
	rename(:x1 => "year", pairs(na)...)
	@subset(!ismissing(:year))
	transform!(n .=> ByRow(float), :year => ByRow(Int), renamecols = false)
	transform!(:RealR => ByRow(x -> (x - 1) * 100), renamecols = false)
	@aside colmetadata!.(Ref(_), first.(realrate_labels), "label", last.(realrate_labels), style=:note)
end

# ╔═╡ e5904152-e553-4e0a-b4e8-78416273cff9
shiller_annual() = outerjoin(realrate_df, annual_hpi_df, on = :year)

# ╔═╡ fbd894cf-71b5-4b55-b6c7-e21ab4e90c0c
dff = @chain shiller_annual() begin
	select(:year, :hpi, :rhpi, :r, :RealR)
	@subset!(:year > 1960)
	stack(Not(:year))
	@subset!(!ismissing(:year), !ismissing(:value))
	@transform!(
		:var1 = :variable ∈ ["hpi", "r"]    ? "nominal" : "real",
		:var2 = :variable ∈ ["hpi", "rhpi"] ? "hpi" : "interest",
	)
	transform!([:variable, :var1, :var2] .=> categorical, renamecols = false)
	disallowmissing
end

# ╔═╡ 69e2c13b-7246-409c-bd22-f7969234c8a6
#=╠═╡
@chain dff begin
...
# ╔═╡ f4e966e2-025e-4664-989f-11d5fa7d3838
describe(dff)

# ╔═╡ 13aa21f5-5d2a-4e41-81ca-a125e2b0b4da
shiller_annual()

# ╔═╡ 150c902b-8ac4-468b-8ab2-dc254f6f8ac4
md"""
## Other stuff
"""

# ╔═╡ 681ee934-8883-4893-9505-34ef05935876
#=╠═╡
TableOfContents()
  ╠═╡ =#
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "1e6a8e3cd0b951a3af6662d653aa8eebe0f2bd83"
